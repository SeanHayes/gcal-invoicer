...
var tests = {
	testProcessEventsOverlappingAndMultiDayEventsTestCase: function(){
		var eventResponse = testEventResponses[0],
			...
			timeMax = moment(events[events.length-1].end.dateTime),
			days = processEvents(events, timeMin, timeMax);
		
		assert(events.length == 10);
		assert(Object.keys(days).length == 5);
		
		assert(JSON.stringify(days) == JSON.stringify({
		"2013-10-01":{
	...
	testMakeInvoice: function(){
		...
		assert(invoice.total, 690);
		assert(invoice.totalCheck, 690);
		assert(invoice.totalTime, 17.25);
	}
}
